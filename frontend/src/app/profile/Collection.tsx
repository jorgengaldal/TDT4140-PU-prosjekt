...
const ImagePoster: React.FC<PosterProps> = ({ link, index, id }) => {
    return (
        <Link href={"/info?id=" + id} className="mx-2">
            <img width={140} height={210} style={{ height: "100%" }} key={index} src={link} alt={`Poster ${index}`} />
        </Link>
    )
};
                ...
                return (
                    <div className="flex flex-row">
                        {likedFilms.slice(0, 5).map((film, index) => (
                            <ImagePoster key={index} link={film.movie.poster} index={index} id={film.movie.imdbid}/>
                        ))}
                    </div>
                );
