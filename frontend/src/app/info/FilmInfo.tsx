...
export default function FilmInfo({ selectedMovieId }: FilmInfoProps) {
    ...
    const handleReviewSubmit = async (review:  { text: string, rating: number }) => {
        //skal håndere submission av reviewform data, sende det til API? 
    }
    ...
    useEffect(() => {
        const fetchMovieData = async () => {
            try {
                ...
                const selectedMovie = allMovies.find((movie) => movie.imdbid === selectedMovieId);
                if (selectedMovie) {
            ...
            <div className="flex flex-row">
                ...
                <div className="p-4 w-4/5">
                    <div className="flex flex-row">
                        ...
                        <h1 className="text-2xl mr-4">{/* content here */}</h1> {/* Add your content here */}
                        <button onClick={handleClickHeart}>
                            <svg xmlns="http://www.w3.org/2000/svg" fill={isClickedHeart ? "white" : "none"} viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6">
                    ...
                    <h1 className="">{movieData && movieData.released}</h1>
                        <h1 className="text-1g mt-2 mx-10">
                            {movieData && movieData.genres.map((genre, index) => (
                                <a key={index} href={"/genre?name=" + genre} className="mx-2">{genre}</a>
                            ))}
                        </h1>
                    </div>
                    <h1 className="mt-3 text-2 mr-4">{movieData && movieData.plot}</h1>
                    <p className="mt-8">Imdb rating:</p> <p>{movieData && movieData.imdbrating}</p>
                </div>
            </div>
            ...
            <div className="w-full relative inset-x-0 bottom-0 bg-accent1 rounded-lg h-[55%]">
                ...
                <div className="p-4">
                    <h1 className="text-1g mt-2 mx-2">
                        {movieData && movieData.actors.map((actor, index) => (
                            <a key={index} href={"/actor?name=" + actor} className="mx-2">{actor}</a>
                        ))}
                    </h1>
                </div>
                <p className="mt-3">Directors:</p>
                <div className="p-4">
                    <h1 className="text-1g mt-2 mx-2">
                        {movieData && movieData.directors.map((director, index) => (
                            <a key={index} href={"/director?name=" + director} className="mx-2">{director}</a>
                        ))}
                    </h1>
                </div>
                <ReviewForm onSubmit={(review) => handleReviewSubmit(review)} />
            </div>
        </div>
    )
}
