  ...
  useEffect(() => {
    ...
    fetch("http://localhost:8000/api/reviews/likednotmovies", {
      ...
      .then((data) => {
        ...
        const likedOfRole = data
          .filter(
            (likedNotMovieObject: LikedNotMovie) =>
              "person_type" in likedNotMovieObject.person
          )
          .map(
            (likedNotMovieObject: LikedNotMovie) => likedNotMovieObject.person
          )
          .filter((person: Person) =>
            person.person_type?.map((type) => type.toLowerCase()).includes(role)
          );
